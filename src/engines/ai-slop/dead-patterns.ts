...
export const detectDeadPatterns = async (context: EngineContext): Promise<Diagnostic[]> => {
	...
	for (const filePath of files) {
		...
		appendAll(diagnostics, detectConsoleLeftovers(codeOnly, relativePath, ext));
		appendAll(diagnostics, detectTodoStubs(content, relativePath));
		appendAll(diagnostics, detectDeadCodePatterns(codeOnly, relativePath, ext));
		// TypeScript suppression directives live in comments, so this one reads raw.
		appendAll(diagnostics, detectUnsafeTypePatterns(content, relativePath, ext));
	}
